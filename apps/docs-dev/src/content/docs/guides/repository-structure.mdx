...
- vexil/
  - apps/
    ...
    - docs-dev/ Dev Docs | VEXiL (Starlight; port 6633)
    - vexil-dev-tools/
      - env-init/ Env initializer (Streamlit + TOML)
...
| Path | Stack | Purpose |
| ---- | ----- | ------- |
| `apps/vexil-io` | Go, Gin | Backend API for the product |
...
| `apps/vexil-dev-tools/env-init` | Python ≥3.14, Streamlit | TOML-driven env scaffolding, workspace init, Houdini package install, patch bumps |
...
| `bun run setup` | `uv sync` + `go mod download`, then launch Streamlit env-init on port `6644` |
| `bun run env-init` | Launch Streamlit env-init UI |
| `bun run bump-version` | Increment product **patch** version across the monorepo |
